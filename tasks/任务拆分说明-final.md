...
- upstream_status: （ready / not_ready）
- gap: （缺失的上游能力是什么）
- strategy: （如何造数或替代验证，如 fixture/仓储状态注入）
- rollback_plan: （上游能力完成后如何回归到正式测试路径）
...
| `test_data_strategy` | 上游未就绪时的测试造数与回归替换方案 | 任务级 |
...
- 优先按真实链路测试；仅当上游能力未就绪时，才允许启用 `test_data_strategy`。
- 替代验证不得通过“弱化断言、跳过用例、删除用例”来换取通过。
- 除“上游未就绪且已声明策略”外，测试失败应先报告并等待人工确认，不可自行强行放行。
