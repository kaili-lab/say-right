...
describe("auth-ui", () => {
  ...
  it("应支持注册表单校验与提交", async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      new Response(JSON.stringify({ user_id: "user-001", email: "kai@example.com" }), {
        status: 201,
    ...
    await waitFor(() =>
      expect(mockFetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: "POST",
          body: JSON.stringify({ email: "kai@example.com", password: "password123" }),
        }),
      ),
    );
  });
  ...
  it("应支持登录并保存令牌", async () => {
    ...
    expect(window.localStorage.getItem("say_right_access_token")).toBe("access-token-demo");
    expect(window.localStorage.getItem("say_right_refresh_token")).toBe("refresh-token-demo");
